...
BavariaDgm1TileDownloader::BavariaDgm1TileDownloader(Config config, FetchFunction fetch)
...
std::string BavariaDgm1TileDownloader::cachePathFor(const TileKey& key) const
{
    return joinPath(m_config.cacheDirectory, fileNameFor(key));
}

bool BavariaDgm1TileDownloader::ensureLocalFile(const TileKey& key, std::string& localPath,
...
bool BavariaDgm1TileDownloader::loadTile(const TileKey& key, Utm32GridTile& tile,
                                         std::string* error) const
{
    ...
    return BavariaDgm1TileReader::readFile(localPath, tile, error);
}

BavariaDgm1HeightDataSource::TileLoader BavariaDgm1TileDownloader::tileLoader() const
{
