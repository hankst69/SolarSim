...
void testAccessors()
{
    ...
    const SolarPosition sun(munich(), time);
...
{
    CHECK_NEAR(SolarPosition(munich(), DateTimeUtc(2024, 6, 21, 12, 0, 0.0)).declination(), 23.44,
               0.1);
    CHECK_NEAR(SolarPosition(munich(), DateTimeUtc(2024, 12, 21, 12, 0, 0.0)).declination(), -23.44,
               0.1);
    CHECK_NEAR(SolarPosition(munich(), DateTimeUtc(2024, 3, 20, 12, 0, 0.0)).declination(), 0.0,
               0.5);
    CHECK_NEAR(SolarPosition(munich(), DateTimeUtc(2024, 9, 22, 12, 0, 0.0)).declination(), 0.0,
               0.5);
}
...
void testNoonElevation()
{
    ...
    const SolarPosition summer(munich(), DateTimeUtc(2024, 6, 21, 11, 15, 0.0));
    CHECK_NEAR(summer.elevation(), 90.0 - kMunichLat + 23.44, 0.5);

    const SolarPosition winter(munich(), DateTimeUtc(2024, 12, 21, 11, 15, 0.0));
...
void testEquatorNoon()
{
    ...
    const SolarPosition sun(GeoLocation(0.0, 0.0), DateTimeUtc(2024, 3, 20, 12, 7, 0.0));
    CHECK_TRUE(sun.elevation() > 88.0);
}

void testNightIsBelowHorizon()
{
    const SolarPosition midnight(munich(), DateTimeUtc(2024, 6, 21, 23, 0, 0.0));
    CHECK_TRUE(midnight.elevation() < 0.0);
    CHECK_FALSE(midnight.isAboveHorizon());
    CHECK_NEAR(midnight.relativeIrradiance(), 0.0, 1e-12);

    const SolarPosition noon(munich(), DateTimeUtc(2024, 6, 21, 11, 15, 0.0));
...
void testPolarDayAndNight()
{
    ...
    for (int hour = 0; hour < 24; hour += 4) {
        const SolarPosition sun(tromso, DateTimeUtc(2024, 6, 21, hour, 0, 0.0));
        CHECK_TRUE(sun.elevation() > 0.0);
    }
    // Midwinter polar night.
    for (int hour = 0; hour < 24; hour += 4) {
        const SolarPosition sun(tromso, DateTimeUtc(2024, 12, 21, hour, 0, 0.0));
        CHECK_TRUE(sun.elevation() < 0.0);
    }
}
...
{
    const SolarPosition sun(munich(), DateTimeUtc(2024, 6, 21, 11, 15, 0.0));
...
void testAzimuthRange()
{
    for (int hour = 0; hour < 24; ++hour) {
        const SolarPosition sun(munich(), DateTimeUtc(2024, 6, 21, hour, 0, 0.0));
...
{
    const SolarPosition morning(munich(), DateTimeUtc(2024, 6, 21, 6, 0, 0.0));
    const SolarPosition noon(munich(), DateTimeUtc(2024, 6, 21, 11, 15, 0.0));
    const SolarPosition evening(munich(), DateTimeUtc(2024, 6, 21, 17, 0, 0.0));
...
void testRefraction()
{
    ...
    const SolarPosition low(munich(), DateTimeUtc(2024, 3, 20, 5, 30, 0.0));
    CHECK_TRUE(low.refractedElevation() > low.elevation());
    CHECK_TRUE(low.refractedElevation() - low.elevation() < 1.0);

    const SolarPosition high(munich(), DateTimeUtc(2024, 6, 21, 11, 15, 0.0));
...
void testEquationOfTime()
{
    ...
    for (int month = 1; month <= 12; ++month) {
        const SolarPosition sun(munich(), DateTimeUtc(2024, month, 15, 12, 0, 0.0));
        CHECK_TRUE(std::fabs(sun.equationOfTime()) < 18.0);
    }
    CHECK_NEAR(SolarPosition(munich(), DateTimeUtc(2024, 2, 11, 12, 0, 0.0)).equationOfTime(),
               -14.2, 1.5);
    CHECK_NEAR(SolarPosition(munich(), DateTimeUtc(2024, 11, 3, 12, 0, 0.0)).equationOfTime(), 16.4,
               1.5);
}
...
void testSunDistance()
{
    ...
    const double perihelion =
        SolarPosition(munich(), DateTimeUtc(2024, 1, 3, 12, 0, 0.0)).sunDistanceAu();
    const double aphelion =
        SolarPosition(munich(), DateTimeUtc(2024, 7, 5, 12, 0, 0.0)).sunDistanceAu();
...
void testHourAngle()
{
    ...
    const SolarPosition afternoon(munich(), DateTimeUtc(2024, 6, 21, 14, 15, 0.0));
...
void testDirection()
{
    const SolarPosition sun(munich(), DateTimeUtc(2024, 6, 21, 9, 0, 0.0));
...
void testProjectOnDome()
{
    ...
    const SolarPosition midnight(munich(), DateTimeUtc(2024, 6, 21, 23, 30, 0.0));
...
void testSouthernHemisphere()
{
    ...
    const SolarPosition sun(sydney, DateTimeUtc(2024, 12, 21, 1, 0, 0.0));
    CHECK_TRUE(sun.elevation() > 0.0);
...
int main()
{
    ...
    return geotest::summarize("SolarPositionTests");
}
