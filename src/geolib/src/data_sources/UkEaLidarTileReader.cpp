...
bool UkEaLidarTileReader::readAsciiGrid(std::istream& stream, BngGridTile& tile, std::string* error)
{
    int columns = 0;
    int rows = 0;
    double xll = 0.0;
    ...
    tile = BngGridTile(originEast, originNorth, cellSize, columns, rows, std::move(grid), noData);
    return true;
}

bool UkEaLidarTileReader::readXyz(std::istream& stream, BngGridTile& tile, std::string* error)
{
    ...
    tile = BngGridTile(minEast, minNorth, spacing, columns, rows, std::move(grid), kNoData);
    return true;
}

bool UkEaLidarTileReader::readFile(const std::string& path, BngGridTile& tile, std::string* error)
{
