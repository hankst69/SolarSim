...
bool UsaUsgs3Dep1mTileReader::readAsciiGrid(std::istream& stream, int zone, UtmGridTile& tile,
                                            std::string* error)
{
    int columns = 0;
    int rows = 0;
    ...
    tile = UtmGridTile(zone, originEast, originNorth, cellSize, columns, rows, std::move(grid),
                       noData);
    return true;
}

bool UsaUsgs3Dep1mTileReader::readXyz(std::istream& stream, int zone, UtmGridTile& tile,
                                      std::string* error)
{
    ...
    tile = UtmGridTile(zone, minEast, minNorth, spacing, columns, rows, std::move(grid), kNoData);
    return true;
}

bool UsaUsgs3Dep1mTileReader::readFile(const std::string& path, int zone, UtmGridTile& tile,
                                       std::string* error)
{
    ...
    if (extension == ".xyz") {
        return readXyz(stream, zone, tile, error);
    }
    return readAsciiGrid(stream, zone, tile, error);
}
